export function mapProductToCard(product) {
  return `
    ...
    <button class="add-to-cart" data-id=${product.id} data-name=${product.name} data-price=${product.price} data-image=${product.imageUrl}>Rezerva acum</button>
    </div>
    `;
}
...
export function mapProductToAdminTableRow(product) {
  return `
            <tr>
               <td>${product.name}</td>
               <td>${product.price}</td>
               <td> <a href="details.html?id=${product.id}">
